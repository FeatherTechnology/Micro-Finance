...
//Today
$today_paid_current = "SELECT COALESCE(COUNT(`sub_status`),0) AS today_paid_current, COALESCE(SUM(total_paid_track),0) AS current_todaypaid FROM `collection` c WHERE `sub_status` ='Payable' AND DATE(created_on) = $currentDate ";
...
$today_fine_od = "SELECT COALESCE(COUNT(`sub_status`),0) AS today_fine_od, COALESCE(SUM(fine_charge_track),0) AS od_todayfine FROM `collection` c WHERE `sub_status` ='OD' AND fine_charge_track !='' AND fine_charge_track != 0  AND DATE(created_on) = $currentDate ";
