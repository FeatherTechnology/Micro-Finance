...
if ($loan_id_calc != '') {
    ...
    $qry = $pdo->query("SELECT gcm.id, cc.cus_id, cc.first_name, gcm.customer_mapping, cc.aadhar_number, cc.mobile1, anc.areaname, gcm.designation ,gcm.loan_amount ,gcm.loan_id ,lelc.loan_status
    ...
    if ($qry->rowCount() > 0) {
        ...
        while ($gcm_info = $qry->fetch(PDO::FETCH_ASSOC)) {
            ...
            $gcm_info['action'] = "</span><span class='icon-trash-2 cusMapDeleteBtn' value='" . $gcm_info['id'] . "' loan_id='" . $gcm_info['loan_id'] . "' loan_status='" . $gcm_info['loan_status'] . "'></span>";
