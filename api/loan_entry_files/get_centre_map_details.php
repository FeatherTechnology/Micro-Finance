...
if ($id != '') {
    ...
    $qry = $pdo->query("SELECT gcm.id, gcm.loan_id, gcm.cus_id as map_id , cc.cus_id, cc.first_name, gcm.customer_mapping, cc.aadhar_number, cc.mobile1, anc.areaname, gcm.designation, gcm.loan_amount, lelc.loan_status, gcm.centre_id
    ...
    if ($qry->rowCount() > 0) {
        ...
        while ($gcm_info = $qry->fetch(PDO::FETCH_ASSOC)) {
            ...
            $gcm_info['action'] = "<span class='icon-trash-2 cusMapDeleteBtn' value='" . $gcm_info['id'] . "'  profile_id='" . $gcm_info['map_id'] . "' loan_id='" . " " . "' loan_status='" . $gcm_info['loan_status'] . "'></span>";
