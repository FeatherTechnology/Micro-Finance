...
$result = '0'; //initial
if ($id != '0' && $id != '') {
    $qry = $pdo->query("UPDATE `scheme` SET `scheme_name`='$add_scheme_name',`due_method`='$scheme_due_method',`benefit_method`='$schemeBenefitMethod',`interest_rate_percent_min`='$scheme_interest_rate_min',`due_period_percent_min`='$scheme_due_period_min',`doc_charge_min`='$scheme_doc_charge_min',`doc_charge_max`='$scheme_doc_charge_max',`processing_fee_min`='$scheme_processing_fee_min',`processing_fee_max`='$scheme_processing_fee_max',`update_login_id`='$user_id',`updated_on`=now() WHERE `id`='$id'");
    if ($qry) {
        $result = '1'; //Update
    }
} else {
    ...
    if ($qry->rowCount() > 0) {
        ...
        $qry = $pdo->query("INSERT INTO `scheme`(`scheme_name`, `due_method`, `benefit_method`, `interest_rate_percent_min`,`due_period_percent_min`,`doc_charge_min`, `doc_charge_max`, `processing_fee_min`, `processing_fee_max`,`scheme_status`, `insert_login_id`, `created_on`) VALUES ('$add_scheme_name','$scheme_due_method','$schemeBenefitMethod','$scheme_interest_rate_min','$scheme_due_period_min','$scheme_doc_charge_min','$scheme_doc_charge_max','$scheme_processing_fee_min','$scheme_processing_fee_max','1','$user_id',now())");
        if ($qry) {
            // Get the inserted ID
