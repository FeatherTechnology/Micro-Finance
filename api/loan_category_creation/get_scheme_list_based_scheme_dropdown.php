...
$scheme_arr = array();
$qry = $pdo->query("SELECT `id`,`scheme_name`, `due_method`, `benefit_method`, `interest_rate_percent_min`, `due_period_percent_min`,`scheme_penalty_type`, `doc_charge_min`, `doc_charge_max`, `processing_fee_min`, `processing_fee_max` FROM `scheme` WHERE FIND_IN_SET(id, '$scheme_id') ");
if ($qry->rowCount() > 0) {
