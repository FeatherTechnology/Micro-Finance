...
$scheme_arr = array();
$qry = $pdo->query("SELECT `id`,`scheme_name`, `due_method`, `benefit_method`, `interest_rate_percent`, `due_period_percent`, `doc_charge_type`, `doc_charge_min`, `doc_charge_max`, `processing_fee_type`, `processing_fee_min`, `processing_fee_max`,`overdue_penalty_percent` FROM `scheme` WHERE FIND_IN_SET(id, '$scheme_id') ");
if ($qry->rowCount() > 0) {
