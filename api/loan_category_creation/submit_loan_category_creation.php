...
if ($loan_cat_creation_id != '') {
    $qry = $pdo->query("UPDATE `loan_category_creation` SET `loan_category`='$loan_category',`loan_limit`='$loan_limit',`profit_type`='$profit_type',`due_method`='$due_method',`due_type`='$due_type',`benefit_method`='$benefit_method',`interest_rate_min`='$interest_rate_min',`interest_rate_max`='$interest_rate_max',`due_period_min`='$due_period_min',`due_period_max`='$due_period_max',`doc_charge_min`='$doc_charge_min',`doc_charge_max`='$doc_charge_max',`processing_fee_min`='$processing_fee_min',`processing_fee_max`='$processing_fee_max',`overdue_penalty`='$overdue_penalty',`penalty_type`='$penalty_type',`scheme_name`='$scheme_name',`status`='$status',`update_login_id`='$user_id',`updated_on`=now() WHERE `id`='$loan_cat_creation_id'");
    if ($qry) {
        $result = 1; //Update
    }
} else {
    $qry = $pdo->query("INSERT INTO `loan_category_creation`(`loan_category`, `loan_limit`, `profit_type`,`due_method`, `due_type`,`benefit_method`, `interest_rate_min`, `interest_rate_max`, `due_period_min`, `due_period_max`, `doc_charge_min`, `doc_charge_max`, `processing_fee_min`, `processing_fee_max`, `overdue_penalty`,`penalty_type`,`scheme_name`,`status`, `insert_login_id`,`created_on`) VALUES ('$loan_category','$loan_limit','$profit_type','$due_method','$due_type','$benefit_method','$interest_rate_min','$interest_rate_max','$due_period_min','$due_period_max','$doc_charge_min','$doc_charge_max','$processing_fee_min','$processing_fee_max','$overdue_penalty','$penalty_type','$scheme_name','$status','$user_id',now())");
    if ($qry) {
        $result = 2; //Insert.
    }
}
