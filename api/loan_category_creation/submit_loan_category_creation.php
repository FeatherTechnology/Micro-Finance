...
if ($loan_cat_creation_id != '') {
    ...
    $qry = $pdo->query("INSERT INTO `loan_category_creation`(`loan_category`, `loan_limit`, `profit_type`,`due_method`, `due_type`,`benefit_method`, `interest_rate_min`, `interest_rate_max`, `due_period_min`, `due_period_max`, `doc_charge_min`, `doc_charge_max`,`procrssing_fees_type`, `processing_fee_min`, `processing_fee_max`, `overdue_penalty`,`penalty_type`,`scheme_name`,`status`, `insert_login_id`,`created_on`) VALUES ('$loan_category','$loan_limit','$profit_type','$due_method','$due_type','$benefit_method','$interest_rate_min','$interest_rate_max','$due_period_min','$due_period_max','$doc_charge_min','$doc_charge_max','$processing_fee_type','$processing_fee_min','$processing_fee_max','$overdue_penalty','$penalty_type','$scheme_name','$status','$user_id',now())");
    if ($qry) {
        $result = 2; //Insert.
    }
}
