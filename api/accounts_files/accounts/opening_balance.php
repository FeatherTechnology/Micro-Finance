...
$sa_cr_h_qry  = $pdo->query("SELECT SUM(amount) AS ot_amnt FROM savings WHERE coll_mode = 1 AND cat_type = 1 AND DATE(created_on) = '$current_date'- INTERVAL 1 DAY  "); //Hand Cash //credit
...
$sa_dr_h_qry = $pdo->query("SELECT SUM(amount) AS ot_amnt FROM savings WHERE coll_mode = 1 AND cat_type = 2 AND DATE(created_on) = '$current_date'- INTERVAL 1 DAY  "); //Hand Cash //debit
